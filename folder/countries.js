...
function topic(array){
	...
	for(let j=0; j<array.length; j++)
		str = array[j-2] + " " + "\"" + array[j-1] + "\""+"("+array[j]+")";
	return str;
}

function outArrayData(info, i, arr){
	let string = "";
	if (typeof(info)=="string")
  {
		if(i==0)
			string += "<th colspan='2'><h1>" + info + " - ";
		else if (i==1)
			string += info + "</h1></th>";
      else
  			string+= "<tr align='center'><td class='about'>" + about[i] + "</td><td class = 'tron'>" + info + "</td></tr>";
	}

	else if (typeof(info)=="number"){
			string+= "<tr><td class = 'about'>" + about[i] + "</td><td class = 'tron'>" + info + "</td></tr>";
	}
	else if (Array.isArray(info)){

		if (!Array.isArray(info[0]))
		{
			string += "</table><table><tr><td class = 'about'>" + about[i] + "</td></tr><ol>";
			for(var j = 0; j<info.length; j++){
				string +="<tr><td></td>" + "<td class = 'tron'><li>" + info[j] + "</li></td></tr>";
			}
			string += "</ol></table>";
		}
		else
		{
			string+="<table><tr><td class = 'about'>" + about[i] +"</td>" +"</tr><ol>";
			for(var j = 0; j<info.length; j++){
				string+="<tr><td></td>" + "<td class = 'tron'>" + "<li>"+ topic(info[j])+"</li>" + "</td></tr>";
			}
			string +="</ol></table>";
		}
	}
	else if (typeof(info)=="object")
	{
		...
		for(j in info){
			string += "<td class='tt'>" + j + " - " + info[j]+"</td>";
		}
		string+="</tr></table>"
	}
	return string;
...
function makeTableFromCountry(country){
	...
	let gObj = country.map(outArrayData);
	for(var i = 0; i<gObj.length; i++){
	tab += gObj[i];
	}
	return tab;
}
let one = document.getElementById("zed");
let out = countries.map(makeTableFromCountry);
